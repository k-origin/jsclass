$(function(){
  ...
  let code_btn = $('.g_navi li:last-child a')
  ...
  //3. 이미지 목록에 마우스 오버 시 캡션 나옴
  g_list.hover(function(){
    $(this).find('span').stop().animate({'bottom':'0px'},300)
  },function(){
    $('.g_list a span').stop().animate({'bottom':'-40px'},300)
  });

  //4. 이미지 클릭 시 해당 src 값을 가져와서 모달 윈도 띄우기
  g_list.click(function(){
    let src = $(this).attr('href');
    let title = $(this).find('span').text(); //내용 가져오기
  ...
  $('.modal i.fa-times').click(function(){
    ...
    //이미지 인덱스 번호가 4, 9, 11일 때 확장자를 png로 교체해 주는 함수
  ...
  //5. 갤러리 메뉴 클릭 시 각각 해당 이미지 목록만 보이게 하기
  plan_btn.click(function(){
    $('.total').hide(); //현재 이미지 모두 숨기기
    $('.plan').fadeIn(); //plan 이미지만 보이기
  });

  design_btn.click(function(){
    $('.total').hide(); //현재 이미지 모두 숨기기
    $('.design').fadeIn(); //design 이미지만 보이기
  });

  ui_btn.click(function(){
    $('.total').hide(); //현재 이미지 모두 숨기기
    $('.ui').fadeIn(); //ui 이미지만 보이기
  });

  code_btn.click(function(){
    $('.total').hide(); //현재 이미지 모두 숨기기
    $('.coding').fadeIn(); //coding 이미지만 보이기
  });
